#!/bin/sh
address="/mount/weili2/zhec/3utr/tissuebytissuesimilarity/data/1e-10/3utr"
for file in /mount/weili2/Lei/GTEx_3UTR-QTLs/*
do
resFile=`echo "$file" | awk -F "/" '{print $NF}'`
resFile2=`echo "$resFile" | awk -F "." '{print $1}'`
awk '$5<=0.0000000001' $file | cut -f1|sort|uniq > $address/3utr."$resFile2".txt &
wait
echo "The cut of $resFile2 is finished"
done
i=0
j=0
pianswer="/mount/weili2/zhec/3utr/tissuebytissuesimilarity/result/matrix_3utr_1e-10.txt"
> $pianswer
for file in $address/*
do
resFile=`echo "$file" | awk -F "/" '{print $NF}'`
