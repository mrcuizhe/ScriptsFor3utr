...
i=0
j=0
pianswer="/mount/weili2/zhec/3utr/tissuebytissuesimilarity/result/matrix_3utr_pair_1e-13.txt"
> $pianswer
for file in $address/*
do
resFile=`echo "$file" | awk -F "/" '{print $NF}'`
...
echo -n $resFile2" " >>$pianswer
let "i += 1"
awk '$3<=0.0000000000001' $file |cut -f1,2 > /mount/weili2/zhec/temp.txt &
#echo $resFile
#echo $pianswer
	if test -f $file
        then
