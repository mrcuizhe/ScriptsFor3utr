#!/bin/sh
address="/mount/weili2/zhec/3utr/tissuebytissuesimilarity/data/1e-10/eqtl"
...
awk '$7<=0.0000000001' $file | cut -f2 | cut -d"_" -f1,2,3,4 |while read a; do echo -e "chr${a}";done|sort|uniq > $address/eqtl."$resFile2".txt &
wait
echo "The cut of $resFile2 is finished"
done
i=0
j=0
pianswer="/mount/weili2/zhec/3utr/tissuebytissuesimilarity/result/matrix_eqtl_1e-10.txt"
> $pianswer
for file in $address/*
do
resFile=`echo "$file" | awk -F "/" '{print $NF}'`
